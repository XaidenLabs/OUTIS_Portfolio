...
const getProjectIcon = (whatFor: string | undefined) => {
  switch (whatFor?.toLowerCase()) {
    case "personal":
      return { icon: <User size={12} className="mr-1" />, label: "Personal" };
    case "client":
      return { icon: <Handshake size={12} className="mr-1" />, label: "Client" };
    case "hackathon":
      return { icon: <FaTrophy size={12} className="mr-1" />, label: "Hackathon" };
    case "bounty":
      return { icon: <FaCoins size={12} className="mr-1" />, label: "Bounty" };
    default:
      return { icon: <Building2 size={12} className="mr-1" />, label: "Project" };
  }
};
...
const ProjectCard = ({
  ...
  const { icon, label } = getProjectIcon(project.whatFor);
  ...
  const getBadgeStyle = (type: string | undefined) => {
    switch (type?.toLowerCase()) {
      ...
      case "client":
        return "bg-emerald-500/90 text-white border border-emerald-400/30";
      case "bounty":
        return "bg-yellow-500/90 text-white border border-yellow-400/30";
      case "personal":
    ...
    >
      <div className="relative h-64 overflow-hidden">
        ...
        <div className="absolute top-4 left-4 z-20">
          <div
            className={`flex items-center px-3 py-1 rounded-full text-xs font-semibold backdrop-blur-sm transition-all duration-300 ${getBadgeStyle(
              project.whatFor
            )}`}
          >
            {icon}
      ...
      <div className="p-8">
        ...
        <div className="flex items-center justify-between">
          ...
          >
            ...
            <ExternalLink
              size={16}
              className="ml-2 group-hover/link:translate-x-1 group-hover/link:-translate-y-1 transition-transform duration-200"
            />
          </Link>
